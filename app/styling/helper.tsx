...
import { GET_BRAIDS_STYLING_COURSES, GET_HEAT_STYLING_COURSES, GET_NEW_STYLING_COURSES, GET_SPECIAL_OCCASION_STYLING_COURSES, GET_TEXTURE_STYLING_COURSES, GET_UPSTYLES_STYLING_COURSES } from "@/graphql/queries/styling";
...
export async function getNewStylingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_NEW_STYLING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.stylingCourses.newStylingCourses;

  return transformDataForWpUrl(courses);
}

export async function getBraidsStylingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_BRAIDS_STYLING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.stylingCourses.braidsStylingCourses;

  return transformDataForWpUrl(courses);
}

export async function getHeatStylingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_HEAT_STYLING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.stylingCourses.heatStylesStylingCourses;

  return transformDataForWpUrl(courses);
}

export async function getSpecialStylingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_SPECIAL_OCCASION_STYLING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.stylingCourses.specialOccasionStylingCourses;

  return transformDataForWpUrl(courses);
}

export async function getTextureStylingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_TEXTURE_STYLING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.stylingCourses.textureStylingCourses;

  return transformDataForWpUrl(courses);
}

export async function getUpstylesStylingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_UPSTYLES_STYLING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.stylingCourses.upstylesAndBridalStylingCourses;
...
export function filterAndSortCourses(courses: Course[]): Course[] {
  ...
  return transformDataForWpUrl(filteredAndSortedCourses);
}
