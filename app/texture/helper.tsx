...
import { GET_BUSINESS_TEXTURE_COURSES, GET_CUTTING_STYLING_TEXTURE_COURSES, GET_HAIRCOLOR_TEXTURE_COURSES, GET_NEW_TEXTURE_COURSES } from "@/graphql/queries/texture";
...
export async function getNewTextureCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_NEW_TEXTURE_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.textureCourses.newTextureCourses;

  return transformDataForWpUrl(courses);
}

export async function getBusinessTextureCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_BUSINESS_TEXTURE_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.textureCourses.businessTextureCourses;

  return transformDataForWpUrl(courses);
}

export async function getCuttingTextureCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_CUTTING_STYLING_TEXTURE_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.textureCourses.cuttingAndStylingTextureCourses;

  return transformDataForWpUrl(courses);
}

export async function getHaircolorTextureCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_HAIRCOLOR_TEXTURE_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.textureCourses.hairColorTextureCourses;
