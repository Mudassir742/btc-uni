import { GET_CATEGORIES_PAGE, GET_CATEGORY_PAGE_GENERAL_DATA_BASICS, GET_CATEGORY_PAGE_GENERAL_DATA_TIPS, GET_CATEGORY_PAGE_GENERAL_DATA_EDUCATORS, GET_CATEGORY_PAGE_GENERAL_DATA_SPOTLIGHT, GET_EDUCATOR_PAGE, GET_USERDATA_BY_DATABASE_ID, GET_CATEGORY_PAGE_GENERAL_DATA_DOWNLOADABLES, GET_CATEGORY_PAGE_GENERAL_DATA_METADATA } from "@/graphql/queries";
import { CategoriesPage, Course, Page, UserData, UserSession, CategoryPageBasics, CategoryPageTips, CategoryPageSpotlight, CategoryPageEducators, CategoryPageDownloadables, Downloadable, AccessedCourse, CategoryPageMetadata } from "@/interfaces";
import { generateWpPageError } from "@/utils/wpErrorHandling";
import { client } from "../../lib/apolloClient";
import { transformDataForWpUrl, transformWpUrl } from "@/utils/url";
import { GET_CONSULTATIONS_BUSINESS_COURSES, GET_INTERVIEW_BUSINESS_COURSES, GET_MGMT_BUSINESS_COURSES, GET_NEW_BUSINESS_COURSES, GET_RECESSION_BUSINESS_COURSES, GET_SERIES_SOCIAL_BUSINESS_COURSES, GET_SOCIAL_CLIMBING_BUSINESS_COURSES, GET_SOCIAL_MEDIA_BUSINESS_COURSES, GET_SOCIAL_MEDIA_BUSINESS_COURSES_EN, GET_SOCIAL_MEDIA_BUSINESS_COURSES_ES, GET_SOCIAL_MEDIA_PHOTOGRAPHY_BUSINESS_COURSES } from "@/graphql/queries/business";
...
export async function getSeriesSocialBusinessCourses(): Promise<Course[]> {
  ...
  const courses: Course[] = data.page.businessCourses.seriesSocialMediaForHairdressers;
...
export async function getSocialMediaBusinessCoursesByLanguage(language: LanguageCode): Promise<Course[]> {
  // Define a mapping of language codes to queries
  const queryMap: { [key in LanguageCode]: DocumentNode } = {
    EN: GET_SOCIAL_MEDIA_BUSINESS_COURSES_EN,
    ES: GET_SOCIAL_MEDIA_BUSINESS_COURSES_ES,
  };

  // Validate language input and select the appropriate query
  const query = queryMap[language];
  // if (!query) {
...
export async function getSocialMediaPhotographyBusinessCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
