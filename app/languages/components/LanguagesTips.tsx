...
interface LanguagesTipsProps {
    user: UserSession | null;
    heroTitle: string;
}

const LanguagesTips: React.FC<LanguagesTipsProps> = async ({ user, heroTitle }) => {

    // check if user is logged in
    const isLoggedIn = (user?.userDataId || 0) > 0;
    
    const tipsCategoryDataPromise = getCategoryPageTips("433293");
