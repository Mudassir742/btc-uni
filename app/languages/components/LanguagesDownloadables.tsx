...
interface LanguagesDownloadablesProps {
  user: UserSession | null;
  heroTitle: string;
}

const LanguagesDownloadables: React.FC<LanguagesDownloadablesProps> = async ({heroTitle, user}) => {

    // check if user is logged in
    const isLoggedIn = (user?.userDataId || 0) > 0;

    const downloadableCategoryDataPromise = getCategoryPageDownloadables("433293");
    ...
    return (
        <div>
          ...
          <div id="Downloadables-section">
            <Section title={`${heroTitle} Resources`} subtitle='' cards={downloadableCards} />
          </div>
        )}
        </div>
    );
...
export default LanguagesDownloadables;