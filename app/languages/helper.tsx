...
import { GET_ALL_LANGUAGES_COURSES, GET_NEW_LANGUAGES_COURSES } from "@/graphql/queries/languages";
...
export async function getCategoryPageMetadata(): Promise<CategoryPageMetadata> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    ...
    body: JSON.stringify({
      ...
      variables: {
        id: `433293`, // languages page wp id
      },
    }),
  });
...
export async function getNewLanguagesCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_NEW_LANGUAGES_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.languagesCourses.newLanguagesCourses;

  return transformDataForWpUrl(courses);
}

export async function getAllLanguagesCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_ALL_LANGUAGES_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.languagesCourses.allLanguagesCourses;
...
export const Section = ({ title, subtitle, cards }: { title: string, subtitle: string, cards: JSX.Element[] }) => (
    <div>
