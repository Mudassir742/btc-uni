...
import { CategoriesPage, CategoryPageBasics, CategoryPageDownloadables, CategoryPageEducators, CategoryPageMetadata, CategoryPageSpotlight, CategoryPageTips, Course, Downloadable, Page, UserData, UserSession } from "@/interfaces";
...
export async function getCategoryPageMetadata(): Promise<CategoryPageMetadata> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    ...
    body: JSON.stringify({
      ...
      variables: {
        id: `433292`, // master classes (aka collections) page wp id
      },
    }),
  });
...
// TO DO: update below func so it for masterclasses (Course Bundles) if we'll ever need this but on Nov 14 Ren said new master classes/collections might not be a thing
...
// : when we add sub tiers, replace purchasedsubscriptions[currentUserData.userDataMetadata.purchasedsubscriptions.length - 1] with more complex code, as users might purchase multiple subscriptions that are concurrently valid
