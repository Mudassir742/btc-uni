...
interface HaircuttingTipsProps {
    user: UserSession | null;
    heroTitle: string;
}

const HaircuttingTips: React.FC<HaircuttingTipsProps> = async ({ user, heroTitle }) => {

    // check if user is logged in
    const isLoggedIn = (user?.userDataId || 0) > 0;
    
    const tipsCategoryDataPromise = getCategoryPageTips("433286");
...
export default HaircuttingTips;