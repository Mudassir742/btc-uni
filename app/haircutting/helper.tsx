...
import { GET_NEW_HAIRCOLOR_COURSES } from "@/graphql/queries/haircolor";
import { GET_BOBS_LOBS_HAIRCUTTING_COURSES, GET_DRY_HAIRCUTTING_COURSES, GET_EXTENSIONS_HAIRCUTTING_COURSES, GET_LAYERS_HAIRCUTTING_COURSES, GET_NEW_HAIRCUTTING_COURSES, GET_PIXIE_HAIRCUTTING_COURSES, GET_RAZOR_HAIRCUTTING_COURSES, GET_SHAGS_HAIRCUTTING_COURSES, GET_TEXTURE_HAIRCUTTING_COURSES } from "@/graphql/queries/haircutting";
...
export async function getCategoryPageMetadata(): Promise<CategoryPageMetadata> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    ...
    body: JSON.stringify({
      ...
      variables: {
        id: `433286`, // haircutting page wp id
      },
    }),
  });
...
export async function getNewHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_NEW_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.newHaircuttingCourses;

  return transformDataForWpUrl(courses);
}

export async function getBobsHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_BOBS_LOBS_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.bobsAndLobsHaircuttingCourses;

  return transformDataForWpUrl(courses);
}

export async function getDryHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_DRY_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.dryCuttingHaircuttingCourses;

  return transformDataForWpUrl(courses);
}

export async function getExtensionsHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_EXTENSIONS_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.extensionsHaircuttingCourses;

  return transformDataForWpUrl(courses);
}

export async function getLayersHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_LAYERS_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.layersHaircuttingCourses;

  return transformDataForWpUrl(courses);
}

export async function getPixieHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_PIXIE_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.pixieHaircuttingCourses;

  return transformDataForWpUrl(courses);
}

export async function getRazorHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_RAZOR_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.razorCuttingHaircuttingCourses;

  return transformDataForWpUrl(courses);
}

export async function getShagsHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_SHAGS_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.shagsHaircuttingCourses;

  return transformDataForWpUrl(courses);
}

export async function getTextureHaircuttingCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_TEXTURE_HAIRCUTTING_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.haircuttingCourses.textureCutsHaircuttingCourses;
