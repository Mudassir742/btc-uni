...
import {
  GET_EVENTS_SHOW_COURSES,
  GET_NEW_EVENTS_COURSES,
  GET_ONESHOT_EVENTS_COURSES,
  GET_ONTOUR_EVENTS_COURSES,
...
export async function getNewEventsCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_NEW_EVENTS_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.btcEventsCourses.newBtcEventsCourses;

  return transformDataForWpUrl(courses);
}

export async function getShowEventsCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_EVENTS_SHOW_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.btcEventsCourses.btcShowCourses;

  return transformDataForWpUrl(courses);
}

export async function getOnTourEventsCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_ONTOUR_EVENTS_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.btcEventsCourses.onTourCourses;

  return transformDataForWpUrl(courses);
}

export async function getOneshotEventsCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_ONESHOT_EVENTS_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.btcEventsCourses.oneshotCourses;
