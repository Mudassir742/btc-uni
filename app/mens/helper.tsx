...
import { GET_CUTTING_MENS_COURSES, GET_FADES_MENS_COURSES, GET_NEW_MENS_COURSES, GET_SCHOREM_MENS_COURSES } from "@/graphql/queries/mens";
...
export async function getNewMensCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_NEW_MENS_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.mensCourses.newMensCourses;

  return transformDataForWpUrl(courses);
}

export async function getCuttingMensCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_CUTTING_MENS_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.mensCourses.cuttingAndStylingMensCourses;

  return transformDataForWpUrl(courses);
}

export async function getFadesMensCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_FADES_MENS_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.mensCourses.fadesAndBarberingMensCourses;

  return transformDataForWpUrl(courses);
}

export async function getSchoremMensCourses(): Promise<Course[]> {
  const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    // cache: "no-store", // we have other options, like "no-store" / no-cache will revalidate the data, with no-store it's checking with each request if there are any changes
    body: JSON.stringify({
      query: GET_SCHOREM_MENS_COURSES.loc?.source.body,
    }),
  });
  ...
  const courses: Course[] = data.page.mensCourses.schoremMensCourses;
