...
interface MensTipsProps {
    user: UserSession | null;
    heroTitle: string;
}

const MensTips: React.FC<MensTipsProps> = async ({ user, heroTitle }) => {

    // check if user is logged in
    const isLoggedIn = (user?.userDataId || 0) > 0;
    
    const tipsCategoryDataPromise = getCategoryPageTips("433289");
