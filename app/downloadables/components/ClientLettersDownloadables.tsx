...
interface ClientLettersDownloadablesProps {
  user: UserSession | null;
  heroTitle: string;
}

const ClientLettersDownloadables: React.FC<ClientLettersDownloadablesProps> = async ({heroTitle, user}) => {

    // check if user is logged in
    const isLoggedIn = (user?.userDataId || 0) > 0;

    const downloadableCategoryDataPromise = getCategoryPageDownloadables("829625"); // updated jan 18 as per new wp page id
...
export default ClientLettersDownloadables;