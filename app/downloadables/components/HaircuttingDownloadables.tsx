...
interface HaircuttingDownloadablesProps {
  user: UserSession | null;
  heroTitle: string;
}

const HaircuttingDownloadables: React.FC<HaircuttingDownloadablesProps> = async ({heroTitle, user}) => {

    // check if user is logged in
    const isLoggedIn = (user?.userDataId || 0) > 0;

    const downloadableCategoryDataPromise = getCategoryPageDownloadables("433286");
    ...
    return (
        <div>
          ...
          <div id="Downloadables-Haircutting-section">
            <Section title={`${heroTitle} Headsheets`} cards={downloadableCards} />
          </div>
        )}
        </div>
    );
...
export default HaircuttingDownloadables;