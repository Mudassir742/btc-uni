...
import { getCategoryPageDownloadables, getCurrentUserDataPurchasedSubscriptions, createActionCards, Section, today, getClientLetters } from '../helper';
import { UserSession } from '@/interfaces';
import { checkSubIsValidSubscriptionObject } from '@/utils/subValidation';
import H3Text from '@/components/text/H3Text';

interface ClientLettersProps {
    user: UserSession | null;
    heroTitle: string;
}

const ClientLetters: React.FC<ClientLettersProps> = async ({ heroTitle, user }) => {

    // check if user is logged in
    const isLoggedIn = (user?.userDataId || 0) > 0;

    const categoryDownloadablesPromise = getClientLetters("829625");
    const userDataPromise = getCurrentUserDataPurchasedSubscriptions(user?.userDataId || 0);

    // fetch user and downloadables in parellel
    const [categoryDownloadables, currentUserData] = await Promise.all([categoryDownloadablesPromise, userDataPromise]);
