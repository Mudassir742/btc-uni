...
import { Section, createActionCards, getCategoryPageDownloadablesTenTips, getCategoryPageTips, getCurrentUserDataPurchasedSubscriptions, today } from '../helper';
import TipParent from '@/components/TipParent';
import { UserSession } from '@/interfaces';
import { checkSubIsValidSubscriptionObject } from '@/utils/subValidation';

interface DownloadablesTenTipsProps {
    user: UserSession | null;
    heroTitle: string;
}

const DownloadablesTenTips: React.FC<DownloadablesTenTipsProps> = async ({ user, heroTitle }) => {

    // check if user is logged in
    const isLoggedIn = (user?.userDataId || 0) > 0;
    
    const tipsCategoryDataPromise = getCategoryPageDownloadablesTenTips("884017"); // updated type of objects and page id jan 18
    const userDataPromise = getCurrentUserDataPurchasedSubscriptions(user?.userDataId || 0);

    // fetch user and downloadables in parellel
    const [categoryDownloadables, currentUserData] = await Promise.all([tipsCategoryDataPromise, userDataPromise]);
    ...
    return (
        <div>
          ...
          <div id="Downloadables-Events-section">
...
export default DownloadablesTenTips;