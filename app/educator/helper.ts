...
import { GET_ALL_EDUCATOR_SLUG, GET_EDUCATOR_BY_SLUG_FOR_EDU_SLUG_PAGE, GET_EDUCATOR_BY_SLUG_FOR_EDU_SLUG_PAGE_METADATA } from "@/graphql/queries/educatorSlugPage";
...
import { Course, Downloadable, Educator, Tip, UserData, UserSession } from "@/interfaces";
...
export async function getEducatorAllSlug(): Promise<string[]> {
    try {
        const response = await fetch(process.env.NEXT_PUBLIC_WORDPRESS_API_URL!, {
            ...
            body: JSON.stringify({
                query: GET_ALL_EDUCATOR_SLUG.loc?.source.body,
            }),
        });
        ...
        const educatorNodes = data?.educators?.nodes;

        if (!Array.isArray(educatorNodes)) {
            throw new Error('Invalid educators data format');
        }

        const slugs: string[] = educatorNodes.map((educator) => {
            return educator?.slug || ''; // Assuming 'slug' property exists on each educator object
        });
        return slugs // Filter out any empty or undefined slugs
    } catch (error) {
        console.error('Error fetching educator slugs:', error);
