...
interface YouTubeVideoCardEducatorsMyCoursesProps {
	classname?: string;
	course: Course;
	completed: boolean;
	themeColor: string;
...
const YouTubeVideoCardEducatorsMyCourses: React.FC<
	YouTubeVideoCardEducatorsMyCoursesProps
> = ({
	classname,
	themeColor,
	completed,
	...
	function formatLaunchDate(dateString: string): string {
		...
		let formattedDate = new Intl.DateTimeFormat("en-US", optionsDate).format(
			date
		);
		let formattedTime = new Intl.DateTimeFormat("en-US", optionsTime)
			.format(date)
			.toLowerCase();
	...
	// Usage
	const formattedLaunchDate = formatLaunchDate(
		courseMetadata?.scheduledreleasedate || ""
	);
	...
	const onUserClick = () => {
		...
		(window as any).dataLayer.push({
			event: "clickedEducatorMyCourse",

			onPageOfEducator: educatorId,
			userDataId: userDataBaseId,

			clickedEducatorCourseId: databaseId,
			clickedEducatorCourseName: title,
	...
	return (
		<div className={cn(`p-4 min-w-[327px] w-full md:w-[370px]`, classname)}>
			{/* <div className="!w-full md:w-[370px]  border-[1px] border-border rounded-xl h-full"> */}
			<div
				className={`!w-full md:w-[370px]  border-[1px] border-border rounded-xl h-full bg-${backgroundColor}`}
			>
				<Link
					id="Educator-My-Course-Card"
					href={`/courses/${slug}`}
					onClick={onUserClick}
				>
              ...
              <YouTubeTrailerVideoPlayer videoId={vimeoPromoId} />
						...
						{isCourseFree && !subscriber && <FreeTextWords />}
					</div>
								...
								{shortDescription ? (
									<div className="line-clamp-3">
										<ParagraphText
											...
											className="line-clamp-3 text-themeColor"
										/>
									</div>
								) : null}
...
export default YouTubeVideoCardEducatorsMyCourses;
